...
```sh
gcloud deployment-manager deployments create boomi-quickstart --config=test_data/config.yaml --labels environment=dev,owner=boomi_molecule
```
...
```sh
gcloud deployment-manager deployments create boomi-quickstart --config=test_data/config_existing_vpc.yaml --labels environment=dev,owner=boomi_molecule
```
